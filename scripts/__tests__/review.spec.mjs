...
const violation = {door: 'structure', severity: 'violation', file: 'src/a.tsx', line: 3, what: 'a div wraps a list', principle: 'lists admit they are lists'};
const concern = {door: 'presentation', severity: 'concern', file: 'src/b.css', line: 9, what: 'a tag selector styles a button', principle: 'Tag selectors are for resets only'};
const note = {door: 'structure', severity: 'note', file: 'src/c.tsx', line: 1, what: 'a section has no heading', principle: 'Sections name themselves through their headings'};
...
describe('the review report', () => {
  ...
  test('an answer without structured findings is refused', () => {
    expect(() => findingsIn(JSON.stringify({type: 'result', result: 'prose'}))).toThrow('structured output is missing');
  });
