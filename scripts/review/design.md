...
- A component keeps what it needs local: its data access, its store, its wiring, its sheet, its page object. A directory is a namespace and the import path reads as one: `@components/DragSortableTable/arrangement` names the arrangement of that table, and importing it is not reaching in. An index is pure: it exports the directory's top-level children by name and does nothing else, no stylesheet, no side effect. An index that carries a side effect, or that re-exports a child module whole under a name so the namespace shows at the use site instead of in the import, is a finding.
