...
export const bannersPage = (page: Page) => {
  ...
  return {
    news,
    leftStack: stack.getByRole('radio', {name: 'Left'}),
    stackToTheLeft: (): Promise<void> => stack.getByText('Left', {exact: true}).click(),
    ...
    settled: (): Promise<boolean> => news.evaluateAll(paragraphs => paragraphs.every(paragraph => {
      ...
      const paddingAndBorders = parseFloat(style.paddingTop) + parseFloat(style.paddingBottom) + parseFloat(style.borderTopWidth) + parseFloat(style.borderBottomWidth);
      return Math.abs(paragraph.getBoundingClientRect().height - (contents.getBoundingClientRect().height + paddingAndBorders)) <= 1;
    }))
  };
};
