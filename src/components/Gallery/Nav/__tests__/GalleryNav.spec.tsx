...
describe('Gallery Navigation', () => {
    ...
    describe('without params', () => {
        beforeEach(() => rendered = renderWithRouter(<GalleryNav/>, Paths.artGallery));
        ...
        describe('from the first page', () => {
            ...
            it('should be able to go to the last page', () => {
                ...
                expect(window.scrollTo).toHaveBeenCalledWith(0, 0);
                expect(rendered().testLocation?.search).toEqual(`?page=${art.pagination.totalPages}`);
        ...
        describe('from the last page', () => {
            ...
            it('should be able to go to the previous page', () => {
                ...
                expect(rendered().testLocation?.search).toEqual(`?page=${art.pagination.totalPages - 1}`);
                expect(window.scrollTo).toHaveBeenCalledWith(0, 0);

                userEvent.click(screen.getByTestId('prev-page'));

                expect(rendered().testLocation?.search).toEqual(`?page=${art.pagination.totalPages - 2}`);
                expect(window.scrollTo).toHaveBeenCalledWith(0, 0);
            });
    ...
    describe('with existing params', () => {
        beforeEach(() => rendered = renderWithRouter(<GalleryNav/>, Paths.artGallery, 'search=q'));

        it('should only update the page query', () => {
            userEvent.click(screen.getByTestId('next-page'));
            expect(rendered().testLocation?.search).toEqual('?search=q&page=2');

            userEvent.click(screen.getByTestId('next-page'));
            expect(rendered().testLocation?.search).toEqual('?search=q&page=3');

            userEvent.click(screen.getByTestId('last-page'));
            expect(rendered().testLocation?.search).toEqual(`?search=q&page=${art.pagination.totalPages}`);

            userEvent.click(screen.getByTestId('prev-page'));
            expect(rendered().testLocation?.search).toEqual(`?search=q&page=${art.pagination.totalPages - 1}`);

            userEvent.click(screen.getByTestId('prev-page'));
            expect(rendered().testLocation?.search).toEqual(`?search=q&page=${art.pagination.totalPages - 2}`);

            userEvent.click(screen.getByTestId('first-page'));
            expect(rendered().testLocation?.search).toEqual('?search=q&page=1');
