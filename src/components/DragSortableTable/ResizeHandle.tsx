...
import {columnGripped, neighbourOfColumn, selectOrder, shareTradedAt, widthOfColumn} from './selectors';
...
export const ResizeHandle: FC<{column: string}> = ({column}) => {
  ...
  const traded = useTableSelector(shareTradedAt(column));
  ...
  return <>
    ...
    <MoveReport landed={maybe(traded).map(resized).orElse(undefined)}/>
  </>;
};
