...
import {Asked, eagerColumnFlight, eagerRowFlight} from '../flights';
import {interior} from '../survey';
import {columnAloft, dealtIn, drifting as drifts, dropped, orderedTo, reseated, rowAloft, seatedTo, standingOf} from '../table-state';
import {grounded, surfaceTravel} from '../travel';
import {ColumnGhost, RowGhost} from '../ghosts';
import {MoveReport} from '../MoveReport';
import '../sortable.css';

export const EagerKeepAnimatedTable: FC<PropsWithChildren> = ({children}) => {
  const store = useStore();
  const values = useValues();
  const [shown, setShown] = useState(store.state());
  useEffect(() => {
    let showing = store.state();
    setShown(showing);
    return store.subscribe(() => {
      const next = store.state();
      if (reseated(showing, next)) {
        glide(true)(() => setShown(next));
      } else {
        setShown(next);
      }
      showing = next;
  ...
  const columnFlight = eagerColumnFlight<Asked>((_asked, held, struck) => settleColumn(held, struck));
  const rowFlight = eagerRowFlight<Asked>((_asked, held, struck) => settleRow(held, struck));
  ...
  return <Shown.Provider value={{state, standing}}>
    ...
    {has(aloft) &&
      <article className="drag-surface"
               onPointerMove={surfaceTravel(drifting, moving => flight.travel(asked, moving), drop)}
               onPointerUp={drop}
