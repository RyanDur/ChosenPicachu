...
const held = (pace: 'eager' | 'lazy', hookSrc: string, dial: ReactNode) => pace === 'eager'
  ? <Step title="Commit inside the move" dial={dial}>
  ...
  : <Step title="Stash the landing, commit on release" dial={dial}>
...
const shown = (origin: 'keep' | 'hide', sources: {table: string; header: string; css: string}, dial: ReactNode) => origin === 'hide'
  ? <Step title="Blank the origin while it is aloft" dial={dial}>
    ...
    <Codes>
      <Snippet label="HTML" lines={[
        ...span(sources.table, 'aloft={columnsTravel.aloft}', 'aloft={columnsTravel.aloft}'), gap,
        ...span(sources.table, 'aloft={rowsTravel.aloft}', 'aloftColumn={columnsTravel.aloft}')
      ]}/>
      <Snippet label="JS" lines={[
        ...span(sources.header, 'const hidden = aloft === columnName;', 'const hidden = aloft === columnName;'),
        aside('// being the hide table is the flag; the element serves itself')
      ]}/>
      <Snippet label="CSS" lines={[
        ...unit(sources.css, '.sortable .hide,'),
        aside('/* the box stops painting; its layout space stays */')
      ]}/>
    </Codes>
  </Step>
  : <Step title="Leave the origin in place while it is aloft" dial={dial}>
...
const moved = (animated: boolean, source: string, cssSrc: string, dial: ReactNode) => animated
  ? <Step title="Slide the theater, not the layout" dial={dial}>
...
const pointerStories = (pace: Pace, origin: Origin, motion: Motion, dials: Dials) => {
  ...
  return <>
    <Story param="sort" id="column"
      ...
      <Tell>This particular table keeps three more promises. {pace === 'eager'
      ...
      <Steps>
        ...
        {held(pace, hookSrc, dials.pace)}
        {shown(origin, {table: source, header: headerSrc, css: cssSrc}, dials.origin)}
        {moved(motion === 'animated', source, cssSrc, dials.motion)}
      </Steps>
    </Story>
...
const nudgedBoth = (animated: boolean, dial: ReactNode) => animated
  ? <Step title="Both parties slide, each by the other’s share" dial={dial}>
  ...
  : <Step title="Cut on the keypress" dial={dial}>
...
const keyedStories = (motion: Motion, dials: Dials) => {
  const headerSrc = headerSources.eager.keep[motion];
  return <Story param="sort" id="keyboard"
    ...
    <Steps>
      ...
      {nudgedBoth(motion === 'animated', dials.motion)}
      {motion === 'animated' &&
...
export const Recipe: FC<Props> = ({track, onTrack, pace, origin, motion, onPace, onOrigin, onMotion}) => {
  ...
  return <section aria-label="build the drag sort yourself" className="build-steps">
    <Picks label="input track"
           ...
           chosen={track}
           onPick={onTrack}/>
    <Stories>{track === 'pointer'
      ? pointerStories(pace, origin, motion, dials)
      : keyedStories(motion, dials)}</Stories>
  </section>;
};
